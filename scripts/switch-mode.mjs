...
const MODES = {
  normal: { name: 'Normal', description: 'Full featured mode with paid models and all agents', template: 'config/opencode-normal.json', launchScript: 'scripts/launch.mjs', color: '\x1b[35m', model: 'opencode-go/deepseek-v4-flash', hasPaidFallbacks: true, agents: ['glitch', 'glitch-omni', 'general', 'explore', 'coder', 'ui-designer', 'reviewer', 'testing', 'vision', 'memory'] },
  free: { name: 'Free', description: 'Free models only (OpenCode Zen, NVIDIA, OpenRouter) - no paid fallbacks', template: 'config/opencode-free.json', launchScript: 'scripts/launch-free.mjs', color: '\x1b[32m', model: 'opencode/deepseek-v4-flash-free', hasPaidFallbacks: false, agents: ['glitch', 'general', 'explore', 'vision', 'glitch-omni', 'memory'] },
  local: { name: 'Local', description: 'Local models via LM Studio (192.168.86.139:1234)', template: 'config/opencode-local.json', launchScript: 'scripts/launch-local.mjs', color: '\x1b[36m', model: 'google/gemma-4-12b', hasPaidFallbacks: false, agents: ['glitch', 'general', 'explore', 'memory'] },
...
async function main() { const args = process.argv.slice(2); if (args.includes('--help') || args.includes('-h')) { console.log('\n  Glitch Mode Switcher\n\n  Usage: node scripts/switch-mode.mjs [mode] [options]\n\n  Modes:\n    normal    Full featured with paid models (default)\n    free      Free models only (OpenCode Zen, NVIDIA, OpenRouter)\n    local     Local models via LM Studio\n    safe      Minimal config for troubleshooting\n\n  Options:\n    --status, -s     Show current mode status\n    --force, -f      Force re-apply current mode\n    --launch, -l     Switch mode AND launch immediately (normal/safe only)\n    --help, -h       Show this help\n\n  Examples:\n    node scripts/switch-mode.mjs           # Interactive mode selection\n    node scripts/switch-mode.mjs free      # Switch to free mode\n    node scripts/switch-mode.mjs --status  # Show current mode\n    node scripts/switch-mode.mjs normal -f # Force re-apply normal mode\n    node scripts/switch-mode.mjs normal -l # Switch to normal and launch\n  '); process.exit(0); }
  ...
  if (!targetMode) { const currentMode = getCurrentMode(); showModeMenu(currentMode); log(WHITE, '  Enter mode number or name (or press Enter to cancel):'); const modeKeys = Object.keys(MODES); let selectedMode; while (true) { const choice = await askQuestion('  > '); if (!choice.trim()) { log(DARK_GRAY, '  Cancelled.'); process.exit(0); } const num = parseInt(choice.trim(), 10); if (!isNaN(num) && num >= 1 && num <= modeKeys.length) { selectedMode = modeKeys[num - 1]; break; } if (MODES[choice.trim().toLowerCase()]) { selectedMode = choice.trim().toLowerCase(); break; } log(RED, '  Invalid selection: ' + choice + '. Please enter a mode number or name.'); } await switchMode(selectedMode, { force, launch }); } else { await switchMode(targetMode.toLowerCase(), { force, launch }); } }
